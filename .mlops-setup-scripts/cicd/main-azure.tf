resource "databricks_group" "mlops-service-principal-group-staging" {
  display_name = "wyn-mlstacks-service-principals"
...
module "azure_create_sp" {
  ...
  service_principal_name       = "wyn-mlstacks-cicd"
  project_directory_path       = "/wyn-mlstacks"
  azure_tenant_id              = var.azure_tenant_id
  service_principal_group_name = "wyn-mlstacks-service-principals"
}
...
provider "databricks" {
  alias = "staging_sp"
  host  = "https://adb-6823129695139011.11.azuredatabricks.net"
  token = module.azure_create_sp.staging_service_principal_aad_token
}

provider "databricks" {
  alias = "prod_sp"
  host  = "https://adb-3539831137511521.1.azuredatabricks.net"
